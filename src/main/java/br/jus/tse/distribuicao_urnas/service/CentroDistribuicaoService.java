...
public class CentroDistribuicaoService {
	...
	public CentroDistribuicaoService(final CentroDistribuicaoRepository centroDistribuicaoRepository,
	...
	public List<CentroDistribuicaoDTO> findAll() {
		return centroDistribuicaoRepository.findAll(Sort.by("id")).stream()
	...
	private CentroDistribuicaoDTO mapToDTO(final CentroDistribuicao centroDistribuicao,
		...
		centroDistribuicaoDTO.setLocalizacao(
				centroDistribuicao.getLocalizacao() == null ? null : centroDistribuicao.getLocalizacao().getId());
		centroDistribuicaoDTO.setTotalDeUrnas(calculaTotalDeUrnas(centroDistribuicao));
		return centroDistribuicaoDTO;
	}

	private Integer calculaTotalDeUrnas(CentroDistribuicao centroDistribuicao) {
		int totalUrnas = 0;
		for (ZonaEleitoral zonaEleitoral : centroDistribuicao.getCentroDistribuicaoZonaEleitorals()) {
			for (LocalVotacao localVotacao : zonaEleitoral.getZonaEleitoralLocalVotacaos()) {
				totalUrnas += localVotacao.getQuantidadeSecoes();
			}
		}
		return totalUrnas;
	}
	...
	private CentroDistribuicao mapToEntity(final CentroDistribuicaoDTO centroDistribuicaoDTO,
		...
		final Localizacao localizacao = centroDistribuicaoDTO.getLocalizacao() == null ? null
				: localizacaoRepository.findById(centroDistribuicaoDTO.getLocalizacao())
	...
	public String getReferencedWarning(final Long id) {
		...
		if (!centroDistribuicao.getCentroDistribuicaoZonaEleitorals().isEmpty()) {
			return WebUtils.getMessage("centroDistribuicao.zonaEleitoral.manyToOne.referenced",
					centroDistribuicao.getCentroDistribuicaoZonaEleitorals().iterator().next().getId());
		} else if (!centroDistribuicao.getCentroDistribuicaoSimulacaos().isEmpty()) {
			return WebUtils.getMessage("centroDistribuicao.simulacao.manyToOne.referenced",
					centroDistribuicao.getCentroDistribuicaoSimulacaos().iterator().next().getId());
		}
		return null;
	}
